...
const EVENT_TERMS = [
  ...
  "Dorffest", "Weinfest", "Food Festival", "verkaufsoffener Sonntag"
];
...
async function nearbyPlaces(center, radiusKm){
  const radiusM = Math.min(Math.max(+radiusKm,10),100)*1000;
  ...
  try{
    const r = await fetch("https://overpass-api.de/api/interpreter", {
      method:"POST",
      headers:{"Content-Type":"application/x-www-form-urlencoded","User-Agent":"Lokale-Veranstaltungsapp/1.0"},
    ...
    for(const el of data.elements||[]){
      ...
      if(dist<=radiusKm){seen.add(name.toLowerCase());list.push({name,dist});}
    }
    return list.sort((a,b)=>a.dist-b.dist).slice(0,8).map(x=>x.name);
  }catch{
    return [center.name];
  }
}
...
async function googleNews(query){
  ...
  const r=await fetch(url,{headers:{"User-Agent":"Lokale-Veranstaltungsapp/1.0"}});
  if(!r.ok) return [];
...
app.get("/api/events", async (req,res)=>{
  try{
    ...
    const radius=Math.min(Math.max(Number(req.query.radius)||50,10),100);
    const page=Math.max(Number(req.query.page)||0,0);
    const category=String(req.query.category||"").trim();
    if(!/^\d{5}$/.test(plz)) return res.status(400).json({error:"Bitte eine fünfstellige deutsche PLZ eingeben."});

    const key=`${plz}|${radius}|${category}`;
    ...
    if(!payload || Date.now()-payload.time>10*60*1000){
      ...
      const primaryPlaces=places.slice(0,6);
      let all=[];
    ...
    res.json({
      ...
      note:"Treffer stammen aus lokalen Nachrichten-/RSS-Suchergebnissen. Ein automatisch erkanntes Veranstaltungsdatum kann fehlen oder ungenau sein; maßgeblich ist die verlinkte Originalquelle."
    });
  }catch(e){
...
app.listen(PORT,()=>console.log(`Lokale Veranstaltungsapp läuft auf Port ${PORT}`));
const express = require("express");
const path = require("path");
